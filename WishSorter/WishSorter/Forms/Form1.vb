...
Public Class mainfrm
    ...
    Private Sub WaitForDoc()
        ...
        Do While isNavigating
            ...
            If timeout >= 20000 Then
                MessageBox.Show("Connection timed out!" & vbNewLine & vbNewLine & "Please make sure you are connected to the internet.", "Error")
                Application.Exit()
            End If
        Loop
    End Sub
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        ...
        'Dim str As String = client.GetAsync("https://www.wish.com/").Result.Content.ReadAsStringAsync.Result

        'Dim html As HtmlAgilityPack.HtmlDocument = New HtmlAgilityPack.HtmlDocument
        'html.LoadHtml(str)

        WebBrowser1.Navigate("www.wish.com")
        ...
        If isLoggedIn Then
            ...
            GoTo goSearch
        Else
            For Each elemnt As HtmlElement In WebBrowser1.Document.GetElementsByTagName("div") 'html.DocumentNode.Descendants("div")
                'If InStr(elemnt.GetAttributeValue("classname", ""), "profile-pic-cont left") Then

                If InStr(elemnt.GetAttribute("classname"), "HeaderBar__HeaderBarAccount") Then
                    ToolStripStatusLabel1.Text = "Status: Already logged in!"
                    isLoggedIn = True
                    GoTo goSearch
                End If
...
goSearch:
        ...
        For Each spanTag As HtmlElement In WebBrowser1.Document.GetElementsByTagName("span")
            If InStr(spanTag.GetAttribute("classname"), "FeedItemV2__Subscript") Then
        ...
        While amountOfItems > 0
            For Each dotloader As HtmlElement In WebBrowser1.Document.GetElementsByTagName("div")
                If InStr(dotloader.GetAttribute("classname"), "WishDotLoader") Then
                    dotloader.ScrollIntoView(False)
                End If
            Next
        ...
        For Each items As HtmlElement In WebBrowser1.Document.GetElementsByTagName("div")
            If InStr(items.GetAttribute("classname"), "ActualPrice") Then ' InStr(items.GetAttribute("classname"), "FeedItem") And
                If Not items.InnerText = "" Then
                    Dim imagelink As String = FunctionsObject.GetImageLink(items.Parent.Parent.Parent.Parent.InnerHtml)

                    If items.InnerText.Contains("Gratis") Then
                        listOfFreeItems.Add(FunctionsObject.GethrefFromImgLink(imagelink) + " " + imagelink)
                    Else
                        listOfItems.Add(New WishItem(FunctionsObject.GethrefFromImgLink(imagelink), items.InnerText.Replace(" " + currencyStr, ""), False, imagelink))
                    End If
                End If
            End If
        Next
        ...
        For Each item As String In listOfFreeItems
            ...
            For Each htmlitem As HtmlElement In WebBrowser1.Document.GetElementsByTagName("div")
                If InStr(htmlitem.GetAttribute("classname"), "ProductTabSection__ShippingPrice") Then
...
goFailCurrency:
    ...
    Private Sub mainfrm_Shown(sender As Object, e As EventArgs) Handles MyBase.Shown
        ...
        If Not isLoggedIn Or email = "" Or password = "" Then
            loginForm.Show()
        End If
    End Sub
